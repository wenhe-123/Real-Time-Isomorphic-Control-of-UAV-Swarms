...
3D plot: ``shared.open_close.morph_lp_plot.update_3d_plot_lp`` (same superellipsoid morph as webcam / Orbbec modes).  Controls: q / p / s.
"""
import argparse
import sys
...
import functions.display_sim.orbbec_hand as ob
...
from functions.mode_switch.modes_runtime import topology_label_from_morph_alpha
from functions.mode_switch.topology_utils import remap_open_display
from functions.open_close.morph_lp_plot import update_3d_plot_lp
...
from functions.dual_cam.dual_view_utils import draw_hand_webcam, open_webcam_capture, overlay_inset
from backup.shared.fusion_utils import fuse_dual_views_weighted, geom_weight_from_eigen_analysis
...
from backup.shared.dual_state_utils import init_dual_runtime_state
...
def main():
    ...
    if shape_norm and not args.no_open_remap:
        lo = demo.OPEN_REMAP_LO if args.open_remap_lo is None else args.open_remap_lo
        hi = demo.OPEN_REMAP_HI if args.open_remap_hi is None else args.open_remap_hi
    ...
    try:
        ...
        while True:
            ...
            mode_raw, tier_count = update_mode_state(
                ...
                classify_mode_fn=demo.classify_mode_from_fingers,
                debounce_frames=demo.MODE_DEBOUNCE_FRAMES,
                mode_smooth=0.22,
            )
            ...
            if snap_state is None:
                ...
                if snap_vis_state is not None:
                    snap_hold_frames += 1
                    if snap_hold_frames >= demo.SNAP_HOLD_AFTER_RELEASE_FRAMES:
                        snap_vis_state = None
                        snap_hold_frames = 0
            else:
                snap_hold_frames = 0
                if snap_vis_state == snap_state:
                    snap_stable_frames = min(demo.SNAP_SHOW_AFTER_FRAMES, snap_stable_frames + 1)
                else:
                    snap_stable_frames += 1
                    if snap_stable_frames >= demo.SNAP_SHOW_AFTER_FRAMES:
            ...
            if enable_3d and (frame_idx % PLOT_EVERY_N_FRAMES) == 0 and hands_3d:
                analyses = update_3d_plot_lp(
                    ...
                    hand_frame=demo.HAND_FRAME_METRIC_MM,
                    hand_3d_source=demo.HAND_3D_SOURCE_MP,
                    hand_frame_palm_plane=demo.HAND_FRAME_PALM_PLANE,
                    norm_axis_halflim=demo.NORM_AXIS_HALFLIM,
                    morph_axis_lim_mm=demo.MORPH_AXIS_LIM_MM,
            ...
            if analyses:
                ...
                need_refresh = (frame_idx % demo.HUD_UPDATE_EVERY_N_FRAMES) == 0 or hud_cache["open"] is None
                if not need_refresh:
                    if abs(float(open_disp) - float(hud_cache["open"])) > demo.HUD_OPEN_STEP:
                        need_refresh = True
                    if abs(float(free_disp) - float(hud_cache["free"])) > demo.HUD_OPEN_STEP:
                        need_refresh = True
                    if abs(float(a0["planarity"]) - float(hud_cache["plan"])) > demo.HUD_METRIC_STEP:
                        need_refresh = True
                    if abs(float(a0["isotropy"]) - float(hud_cache["iso"])) > demo.HUD_METRIC_STEP:
                        need_refresh = True
                    if abs(float(a0["finger_spread"]) - float(hud_cache["spread"])) > demo.HUD_METRIC_STEP:
                        need_refresh = True
