...
def draw_pyramid_morph_canonical(ax, radius, open_alpha, show_refs=True):
    """
    Square pyramid: four triangular side faces use semi-transparent colored surfaces + wireframe
    (similar to box); base is light wireframe only.

    Apex at ``(0,0,z_apex)`` with ``z_apex = close·H``; when flat (close→0), sides collapse into
    fan triangles sharing the origin on the base. Black sample points lie only on the four side
    faces (``MORPH_PLANE_GRID_N²`` points distributed by area).
    """
    open_alpha = _clamp01(open_alpha)
