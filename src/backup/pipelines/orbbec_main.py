...
from functions.mode_switch.topology_utils import clamp01, remap_open_display
...
def build_parser() -> argparse.ArgumentParser:
    ...
    ap.add_argument("--depth-fusion", type=float, default=demo.DEPTH_FUSION_WEIGHT, help="0=MediaPipe world only, 1=depth unproject only")
    ...
    ap.add_argument("--open-remap-lo", type=float, default=None, metavar="X", help=f"Lower bound for open remap (default {demo.OPEN_REMAP_LO}).")
    ap.add_argument("--open-remap-hi", type=float, default=None, metavar="X", help=f"Upper bound for open remap (default {demo.OPEN_REMAP_HI}).")
    ...
    ap.add_argument("--hand-3d", choices=(demo.HAND_3D_SOURCE_MP, demo.HAND_3D_SOURCE_FUSED), default=demo.HAND_3D_SOURCE_MP, help="3D plot source: mp | fused")
...
def _print_periodic_metrics(frame_idx: int, hand_frame: str, hands_3d, analyses, open_out, open_free_ema, open_remap, morph_mode: int, mode_raw: int):
    ...
    if open_remap is not None:
        ...
        out_show = remap_open_display(out_v, lo_r, hi_r)
        free_show = remap_open_display(free_v, lo_r, hi_r)
    ...
    curl_part = ""
    if hand_frame == demo.HAND_FRAME_PALM_PLANE and hands_3d:
...
def run(config: OrbbecModesConfig):
    ...
    runtime = RuntimeState(enable_3d=demo.ENABLE_3D_PLOT)
    ...
    try:
        while True:
            ...
            if (
                ...
                and config.hand_3d == demo.HAND_3D_SOURCE_FUSED
            ...
            analyses = None
            if runtime.enable_3d and (runtime.frame_idx % demo.PLOT_EVERY_N_FRAMES) == 0 and hands_3d:
