...
from functions.mode_switch.modes_runtime import ModeState, update_mode_state
...
def test_mode_from_webcam_when_visibility_low_and_dual_assist():
    st = ModeState()
    st.morph_mode = 2
    st.last_mode_raw = 2
    st.mode_raw_prev = 5
    st.mode_stable_frames = 1
    wpts = [(0.0, 0.0, 0.0)] * 21
    raw, _ = update_mode_state(
        wpts,
        mode_state=st,
