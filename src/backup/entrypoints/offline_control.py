...
_BACKUP_ROOT = Path(__file__).resolve().parents[1]
DEFAULT_LOG_PATH = _BACKUP_ROOT / "data" / "logs" / "text3.txt"
DEFAULT_PLOT_PATH = _BACKUP_ROOT / "data" / "plots" / "offline_control_trajectory.png"


def resolve_log_path(log_path: Path) -> Path:
    """Resolve log file; try ``backup/data/logs/`` for bare filenames."""
    p = Path(log_path)
    if p.is_file():
        return p
    if not p.is_absolute() and p.parent == Path("."):
        alt = _BACKUP_ROOT / "data" / "logs" / p.name
        if alt.is_file():
            return alt
    return p
...
def infer_point_count(log_path: Path) -> int:
    """Pick the ``n`` with the most complete ``points=`` frames in the log."""
    ...
    for line in log_path.read_text(encoding="utf-8").splitlines():
        ...
        pts = np.full((n, 3), np.nan, dtype=float)
        ...
        if np.isfinite(pts).all():
            complete[n] = complete.get(n, 0) + 1
    if not complete:
        raise ValueError(f"No complete morph point frames found in {log_path}")
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="Drone / morph count (0 = auto-detect from log, e.g. n=24 in text3.txt).",
    )
    ...
    log_path = resolve_log_path(Path(args.log))
    if not log_path.is_file():
        raise FileNotFoundError(
            f"Log not found: {args.log} (also tried backup/data/logs/ for bare names)"
        )

    point_count = int(args.point_count)
    if point_count <= 0:
        point_count = infer_point_count(log_path)
        print(f"Auto-detected point-count from log: n={point_count}")

    points_mm = load_points(log_path, point_count=point_count)
