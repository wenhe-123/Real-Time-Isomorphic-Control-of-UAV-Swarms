...
from shared.depth_fusion_utils import project_depth_cam_mm_to_color_px
from shared.left_hand_swarm_pose import palm_orthonormal_basis, rotvec_to_R
...
def draw_depth_cam_basis_on_bgr(
    ...
    *,
    ...
    """Draw RGB = XYZ columns of ``basis`` (3×3) at ``wrist_mm`` in depth camera mm."""
    if calibration is None or frame is None:
        return
    h, w = int(frame.shape[0]), int(frame.shape[1])
    o = project_depth_cam_mm_to_color_px(calibration, wrist_mm)
    if o is None:
        return
    ...
    wrist = np.asarray(wrist_mm, dtype=np.float64).reshape(3)
...
def draw_left_pose_frame_overlay(
    ...
    *,
    ...
    """Hand frame + swarm pose arrows (panel + optional projection at palm)."""
    if frame is None:
        return
    armed = bool(left_runtime_armed or left_pose_state.is_unwinding())
    mot = motion if armed else "none"
    rv_pose = (
        np.asarray(getattr(left_pose_state, "last_rv_pose_world", None), dtype=np.float64).reshape(3)
        if armed and getattr(left_pose_state, "last_rv_pose_world", None) is not None
        else None
    )
    rv_cmd = (
        np.asarray(getattr(left_pose_state, "last_rv_cmd_world", None), dtype=np.float64).reshape(3)
        if armed and getattr(left_pose_state, "last_rv_cmd_world", None) is not None
        else None
    )
    draw_swarm_pose_panel_on_bgr(
        frame,
        R_pose=R_pose if armed else None,
        off_m=off_m if armed else None,
        motion=mot,
        rv_pose_world=rv_pose,
        rv_cmd_world=rv_cmd,
        pose_rotate_rad=pose_rotate_rad,
        armed=armed,
    )
    draw_swarm_centroid_frame_inset(
        frame,
        R_pose=R_pose if armed else None,
        off_m=off_m if armed else None,
        armed=armed,
    ...
    if (
        armed
        and R_pose is not None
        and use_depth_projection
        and calibration is not None
        and pts_l_pose_mm is not None
        and getattr(left_pose_state, "frozen_M_rot", None) is not None
    ):
        out = palm_orthonormal_basis(np.asarray(pts_l_pose_mm, dtype=np.float64), palm_basis=palm_basis)
        if out is not None:
            palm_o, _ = out
            draw_swarm_world_axes_at_palm_on_bgr(
                frame,
                calibration,
                palm_o,
                R_pose,
                left_pose_state.frozen_M_rot,
                off_m=off_m,
            )
    if pts_l_pose_mm is not None:
        out = palm_orthonormal_basis(np.asarray(pts_l_pose_mm, dtype=np.float64), palm_basis=palm_basis)
        if out is not None:
            origin, B_now = out
            o_draw = (
                np.asarray(left_pose_state.ref_palm_center, dtype=np.float64).reshape(3)
                if left_pose_state.initialized and armed
                else origin
            )
            if use_depth_projection and calibration is not None:
                ...
                if left_pose_state.initialized and armed:
                    draw_depth_cam_basis_on_bgr(
                        ...
                        calibration,
                        o_draw,
                        left_pose_state.ref_basis,
                        axis_len_mm=48.0,
                        prefix="0",
                        thickness=1,
                    )
            elif result is not None and idx_l is not None:
                draw_image_plane_basis_on_bgr(
                    frame, result, idx_l, B_now, prefix="h", thickness=2
                )
    elif B_rot is not None and result is not None and idx_l is not None:
        draw_image_plane_basis_on_bgr(frame, result, idx_l, B_rot, prefix="h", thickness=2)
        if left_pose_state.ref_basis_image is not None and armed:
            draw_image_plane_basis_on_bgr(
                frame,
                result,
                idx_l,
                left_pose_state.ref_basis_image,
                prefix="0",
                thickness=1,
            )
