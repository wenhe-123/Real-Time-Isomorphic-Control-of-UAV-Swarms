"""Background key input for online_control (does not require Orbbec OpenCV window focus).

Uses ``pynput`` or ``keyboard`` global hooks when available; otherwise a cbreak stdin thread
on the controlling terminal (focus the terminal where you launched ``pixi run …``).

OpenCV GUI is **not** thread-safe — we never call ``imshow``/``waitKey`` from a background thread.
"""
