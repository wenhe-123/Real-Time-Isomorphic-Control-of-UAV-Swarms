...
def test_palm_basis_is_orthonormal_right_handed():
    ...
    h[IDX_WRIST] = wrist
    h[IDX_MIDDLE_MCP] = wrist + np.array([4.0, -58.0, 6.0])
    h[IDX_MIDDLE_TIP] = wrist + np.array([5.0, -98.0, 10.0])
    h[IDX_THUMB_MCP] = wrist + np.array([-44.0, -42.0, 4.0])
    h[IDX_THUMB_TIP] = wrist + np.array([-62.0, -36.0, 6.0])
...
def test_trans_rot_blend_keeps_secondary():
    from shared.left_hand_swarm_pose import axis_locked_trans_rot_blend_weights
